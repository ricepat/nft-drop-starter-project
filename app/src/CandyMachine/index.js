...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getProvider = () => {
    ...
    //create new Solana provider obj
  ...
  const getCandyMachineState = async () => {
    ...
    //generate timestring for use in UI later
    ...
    if (data.length !== 0) {
      for (const mint of data) {
        ...
        console.log("Past Summoned Familiars", mint);
  ...
  const renderMintedItems = () => (
    <div className="gif-container">
      <p className="sub-text">Summoned Familiars ✨</p>
      <div className="gif-grid">
        {mints.map((mint) => (
          <div className="gif-item" key={mint}>
            <img src={mint} alt={`Summoned Familiar ${mint}`} />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    <div className="machine-container">
      ...
      <p>{`Familiars Summoned: ${machineStats.itemsRedeemed} / ${machineStats.itemsAvailable}`}</p>
      {machineStats.itemsRedeemed === machineStats.itemsAvailable ? (
        <p className="sub-text">The Summoning Ceremony is Complete! Better luck next time!</p>
      ) : (
        <button 
      ...
      {isLoadingMints && <p>LOADING FAMILIARS...</p>}
    </div>
  ));
};
